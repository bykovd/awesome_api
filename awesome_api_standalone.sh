#!/bin/bash
sudo apt-get update
sudo apt-get -y install git-core curl zlib1g-dev build-essential libssl-dev libreadline-dev libyaml-dev libsqlite3-dev sqlite3 libxml2-dev libxslt1-dev libcurl4-openssl-dev python-software-properties libffi-dev nodejs libpq-dev postgresql-client mc  redis-server postgresql-9.5 python-pip
...
sudo -u postgres psql -c "CREATE DATABASE my_api_development;"
sudo -u postgres psql -c "CREATE DATABASE my_api_test;"
sudo -u postgres psql -c "CREATE USER awesome_api_db_user WITH password 'awesome_api_db_pass';"
sudo -u postgres psql -c "GRANT ALL privileges ON DATABASE my_api_development TO awesome_api_db_user;"
sudo -u postgres psql -c "GRANT ALL privileges ON DATABASE my_api_test TO awesome_api_db_user;"

cat <<EOF > /opt/awesome_api/api/.env
DATABASE_URL=postgres://awesome_api_db_user:awesome_api_db_pass@127.0.0.1/my_api_development
RACK_ENV=development
REDIS_PUBLISH_URL=redis://127.0.0.1:6379/0
RDS_DB_NAME= my_api_development
RDS_HOSTNAME=127.0.0.1
RDS_PASSWORD=awesome_api_db_pass
RDS_PORT=5432
RDS_USERNAME=awesome_api_db_user
TEST_DATABASE_URL=postgres://awesome_api_db_user:awesome_api_db_pass@127.0.0.1/my_api_test
EOF
...
DB ||= if ENV["RACK_ENV"] == "test"
         Sequel.connect('postgres://awesome_api_db_user:awesome_api_db_pass@127.0.0.1/my_api_test')
       elsif ENV.key?("RDS_HOSTNAME")
         Sequel.connect('postgres://awesome_api_db_user:awesome_api_db_pass@127.0.0.1/my_api_development')
       else
         Sequel.connect('postgres://awesome_api_db_user:awesome_api_db_pass@127.0.0.1/my_api_development')
       end
